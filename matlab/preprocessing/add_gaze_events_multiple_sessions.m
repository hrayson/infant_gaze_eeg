function data=add_gaze_events_multiple_sessions(data, age, subj_id, coder, num_sessions, fps, varargin)
%%
...
% Trial offset - because EEG contains trials from both sessions
trial_offset=0;

% Loop through each session
for session=1:num_sessions
    % Read in initial direction coding - direction the infant is looking
    % in when the object highlighting starts (ima2) along with trial type
    init_dir_fid=fopen(fullfile(raw_data_dir,...
        sprintf('%d%s%s-initialdirectioncoding-%d.csv', subj_id, coder,...
        age, session)));
...
%         trials(trial_num).shuf=shuf;
    ...
    %    offset_trial_idx=trial_offset+trial_idx;
    ...
    %                if mov1_evts_seen==offset_trial_idx                
    ...
    mvmt_fid=fopen(fullfile(raw_data_dir,...
        sprintf('%d%s%s-movementcoding-%d.csv', subj_id, coder, age,...
        session)));
    ...
    for i=2:length(lines{1})
        ...
        for trial_idx=1:length(trials)
            ...
            if (trial_idx==length(trials) && onset_frame>=trials(trial_idx).ima2_frame) ||  ... % If this is the last trial and movement occurred after ima2 event
                ...
                % index of trial relative to whole experiment (not just current session)
                offset_trial_idx=trial_offset+trial_idx;
...
%                 mvmt_pat='antitarget';
%                 if strcmp(mvmt_dir,trials(trial_idx).direction)
%                     mvmt_pat='target';
...
%                 end
                ...
                for evt_idx=1:length(data.event)
                    ...
                    elseif strcmp(data.event(evt_idx).type,'mov1')
                        ...
                        mov1_evts_seen=mov1_evts_seen+1;
                        % If mov1 events seen equal to current movement
                        % trial idx (relative to whole experiment)
                        if mov1_evts_seen==offset_trial_idx
                ...
                data = pop_editeventvals(data,'append',{1 mvmt_type ...
                    (ima2_latency_sec+mvmt_start_rel_sec) ...
                    length(data.event)+1 shuf mvmt_dir ...
                    'none' mvmt_pat congruence ...
                    mvmt_duration_sec});
    ...
    % Update trial offset - add number of trials in this session
    trial_offset=trial_offset+length(trials);
end
