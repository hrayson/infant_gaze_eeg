function prepare_coded_eeg_data(subjects, exclude_head_turns)

% File to write all data to
fname='/home/jbonaiuto/Dropbox/joint_attention/infant_gaze_eeg/coded_eeg/all_subjects.csv';
if exclude_head_turns
    fname='/home/jbonaiuto/Dropbox/joint_attention/infant_gaze_eeg/coded_eeg/all_subjects_no_headturns.csv';
end
fid=fopen(fname,'w');
fwrite(fid, sprintf('Subject,Age,Condition,Congruence,Trial,TrialTime,Target,AntiTarget,Face,LeftObject,RightObject,Trackloss,Message\n'));
    ...
    for age_idx=1:length(subj_info.ages)
        ...
        if num_runs>1
            ...
            for run_num=1:num_runs
                ...
                for trial_idx=1:length(trials)
                    trials(trial_idx).fixations=[];
                    if ~strcmp(trials(trial_idx).init_dir,'offscreen')
                        trials(trial_idx).fixations(1).start_frame=0;
                        trials(trial_idx).fixations(1).end_frame=-1;
                        if strcmp(trials(trial_idx).init_dir,'centre')
                            trials(trial_idx).fixations(1).mvmt_pat='face';
                        elseif strcmp(trials(trial_idx).init_dir, trials(trial_idx).direction)
                            trials(trial_idx).fixations(1).mvmt_pat='target';
                        else
                            trials(trial_idx).fixations(1).mvmt_pat='antitarget';
                        end
                    end
                    for mvmt_idx=1:length(trials(trial_idx).movements)
                        ...
                        fixation_num=length(trials(trial_idx).fixations)+1;
                        trials(trial_idx).fixations(fixation_num).start_frame=trials(trial_idx).movements(mvmt_idx).end;
                        trials(trial_idx).fixations(fixation_num).end_frame=-1;
                        if fixation_num>1
                            trials(trial_idx).fixations(fixation_num-1).end_frame=trials(trial_idx).movements(mvmt_idx).start;
                        end
                        trials(trial_idx).fixations(fixation_num).mvmt_pat=trials(trial_idx).movements(mvmt_idx).mvmt_pat;
                    end
                    if length(trials(trial_idx).fixations)>0 && trials(trial_idx).fixations(end).end_frame<0
                        trials(trial_idx).fixations(end).end_frame=trials(trial_idx).end_frame-trials(trial_idx).ima2_frame;
                    elseif length(trials(trial_idx).fixations)==0
                        disp(sprintf('%d %s trial %d has no fixations!', subj_info.subj_id, age, trial_idx));
                    end
                end
                ...
                for trial_num=1:length(included_trials)
                    ...
                    num_frames=trials(trial_idx).end_frame-trials(trial_idx).ima2_frame;
                    for frame=1:num_frames
                        ...
                        for fixation_idx=1:length(trials(trial_idx).fixations)
                            if frame>=trials(trial_idx).fixations(fixation_idx).start_frame && frame<=trials(trial_idx).fixations(fixation_idx).end_frame
                                mvmt_pat=trials(trial_idx).fixations(fixation_idx).mvmt_pat;
                                break;
                            end
                        end
                        trackloss='FALSE';
                        if strcmp(mvmt_pat,'offscreen')
                            trackloss='TRUE';
                        end
                        target='FALSE';
                        if strcmp(mvmt_pat,'target')
                            target='TRUE';
                        end
                        antitarget='FALSE';                 
                        if strcmp(mvmt_pat,'antitarget')
                            antitarget='TRUE';
                        end
                        face='FALSE';
                        if strcmp(mvmt_pat,'face')
                            face='TRUE';
                        end
                        leftObject='FALSE';
                        if (strcmp(mvmt_pat,'target') && strcmp(trials(trial_idx).direction,'left')) || (strcmp(mvmt_pat,'antitarget') && strcmp(trials(trial_idx).direction,'right'))
                            leftObject='TRUE';
                        end
                        rightObject='FALSE';
                        if (strcmp(mvmt_pat,'target') && strcmp(trials(trial_idx).direction,'right')) || (strcmp(mvmt_pat,'antitarget') && strcmp(trials(trial_idx).direction,'left'))
                            rightObject='TRUE';
                        end
                        message='';
                        if frame==trials(trial_idx).mov1_frame
                            message='mov1';
                        end
                        fwrite(fid, sprintf('%d_%s,%s,%s,%s,%d,%0.4f,%s,%s,%s,%s,%s,%s,%s\n', subj_info.subj_id, age, age, trials(trial_idx).condition, trials(trial_idx).congruence, trial_num+block_offset, time_s, target, antitarget, face, leftObject, rightObject, trackloss, message));
                    end
                end
            ...
            for trial_num=1:length(included_trials)
                ...
                for frame=1:num_frames
                    ...
                    fwrite(fid, sprintf('%d_%s,%s,%s,%s,%d,%0.4f,%s,%s,%s,%s,%s,%s,%s\n', subj_info.subj_id, age, age, trials(trial_idx).condition, trials(trial_idx).congruence, trial_num, time_s, target, antitarget, face, leftObject, rightObject, trackloss, message));
                end
            end
        end
    end
