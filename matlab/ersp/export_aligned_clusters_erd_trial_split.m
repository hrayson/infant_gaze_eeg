function export_aligned_clusters_erd_trial_split(conditions, freq_bands, wois, baseline_woi, min_trials_per_cond,...
...
fprintf(fid, 'Subject,Age,Region,Hemisphere,WOI,FreqBand,Condition,Half,ERD\n');
    ...
    for freq_idx=1:length(freq_bands)
        ...
        if strcmp(freq_band.age,'') || strcmp(freq_band.age,'6m')
            ...
            for w=1:size(wois,1)
                ...
                [early_erds, early_subjs, early_subj_idx, late_erds, late_subjs, late_subj_idx]=compute_condition_erds_trial_split(conditions, ...
                ...
                for cond_idx=1:length(conditions)
                    ...
                    for subj_idx=1:length(early_subjs)
                        subj_id=early_subjs(subj_idx);
                        fprintf(fid, '%d,6m,%s,%s,%d-%dms,%s,%s,early,%.4f\n', subj_id, ...
                            cluster.region, cluster.hemisphere, woi(1), woi(2), freq_band.name, ...
                            condition, early_erds(cond_idx,subj_idx));
                    end
                end
                for cond_idx=1:length(conditions)
                    condition=conditions{cond_idx};
                    for subj_idx=1:length(late_subjs)
                        subj_id=late_subjs(subj_idx);
                        fprintf(fid, '%d,6m,%s,%s,%d-%dms,%s,%s,late,%.4f\n', subj_id, ...
                            cluster.region, cluster.hemisphere, woi(1), woi(2), freq_band.name, ...
                            condition, late_erds(cond_idx,subj_idx));
                    end
                end
            end
        end
        ...
        if strcmp(freq_band.age,'') || strcmp(freq_band.age,'9m')
            ...
            for w=1:size(wois,1)
                ...
                for cond_idx=1:length(conditions)
                    ...
                    for subj_idx=1:length(early_subjs)
                        subj_id=early_subjs(subj_idx);
                        fprintf(fid, '%d,9m,%s,%s,%d-%dms,%s,%s,early,%.4f\n', subj_id, ...
                            cluster.region, cluster.hemisphere, woi(1), woi(2), freq_band.name, ...
                            condition, early_erds(cond_idx,subj_idx));
                    end
                end
                for cond_idx=1:length(conditions)
                    ...
                    for subj_idx=1:length(late_subjs)
                        subj_id=late_subjs(subj_idx);
                        fprintf(fid, '%d,9m,%s,%s,%d-%dms,%s,%s,late,%.4f\n', subj_id, ...
                            cluster.region, cluster.hemisphere, woi(1), woi(2), freq_band.name, ...
                            condition, late_erds(cond_idx,subj_idx));
                    end
                end
