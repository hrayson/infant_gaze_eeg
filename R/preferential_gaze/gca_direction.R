gca_direction <- function(bin_size, non_aoi_missing, data_file, output_dir) {
	...
	eyetrackingr_data <- make_eyetrackingr_data(data, participant_column = "Subject", trial_column = "Trial", time_column = "TrialTime", trackloss_column = "Trackloss", aoi_columns = c('Left', 'Right'), treat_non_aoi_looks_as_missing = non_aoi_missing, item_columns = c('Block'))
	...
	response_time <- make_time_sequence_data(response_window_clean, time_bin_size = bin_size, predictor_columns = c("Block","Age"), aois = c("Left","Right"))

	# Crazy plotting stuff - plot timecourse with age
	dev.new()
	data_options = attr(response_time, "eyetrackingR")$data_options
	df_plot <- group_by_.time_sequence_data(response_time, .dots = c(data_options$participant_column, "Time", "AOI", "Block", "Age"))
	...
	# Create predictor for face type = .5 if Left, -.5 if Right
	response_time$FaceTypeC <- ifelse(response_time$AOI == 'Left', .5, -.5)
