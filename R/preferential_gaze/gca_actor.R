gca_actor <- function(bin_size, non_aoi_missing, data_file, output_dir) {
	...
	eyetrackingr_data <- make_eyetrackingr_data(data, participant_column = "Subject", trial_column = "Trial", time_column = "TrialTime", trackloss_column = "Trackloss", aoi_columns = c('FO', 'CG'), treat_non_aoi_looks_as_missing = non_aoi_missing, item_columns = c('Block'))
	...
	response_time <- make_time_sequence_data(response_window_clean, time_bin_size = bin_size, predictor_columns = c("Block","Age"), aois = c("FO","CG"))
	...
	# Create predictor for face type = .5 if FO, -.5 if CG
	response_time$FaceTypeC <- ifelse(response_time$AOI == 'FO', .5, -.5)
