shuffled_window_analysis <- function(non_aoi_missing, data_file, output_dir) {
	...
	trackloss$Congruence<-''
	for(i in 1:nrow(trackloss)) {
		subj <- trackloss$Subject[i]
		trial <- trackloss$Trial[i]
		trackloss$Congruence[i] <- as.character(response_window$Congruence[response_window$Subject==subj & response_window$Trial==trial & response_window$Message=='mov1'])
        }
	trackloss$Congruence<-as.factor(trackloss$Congruence)
	trackloss_model<-lmer(TracklossSamples ~ Congruence + (1|Subject), data=trackloss, REML=FALSE, control = lmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e6)))
	...
	data_summary <- describe_data(response_window_clean, describe_column = 'Face', group_columns = c('Congruence', 'Subject'))
	print(data_summary)
	...
	# Sanity checks. Looks at some descriptive statitics 
	data_summary <- describe_data(response_window_clean, describe_column = 'Target', group_columns = c('Congruence', 'Subject'))
	print(data_summary)
	...
	# Sanity checks. Looks at some descriptive statitics 
	data_summary <- describe_data(response_window_clean, describe_column = 'AntiTarget', group_columns = c('Congruence', 'Subject'))
	print(data_summary)
	...
	response_window_agg_by_subject <- make_time_window_data(response_window_clean, aois= c('Face', 'Target', 'AntiTarget'), predictor_columns=c('Congruence', 'Age'),summarize_by="Subject")

	# Plots mean looking time to angry and sad per subject. Note: used ArcSin transofrmation, but can use others (e.g Elog).
	dev.new()
	p<-plot(response_window_agg_by_subject, predictor_columns= c('Congruence', 'Age'), dv="ArcSin")
	print(p)
	...
	print(describe_data(response_window_agg_by_subject,describe_column = "ArcSin", group_columns= c('AOI','Congruence', 'Age')))


	# Averages proportion of looks over trials (makes in eyetrakingR format without aggregating over subject)
	response_window_agg <- make_time_window_data(response_window_clean, aois= c('Face', 'Target', 'AntiTarget'), predictor_columns=c('Congruence', 'Age'))


	# Mixed model
	model_time_window_logit <- lmer(LogitAdjusted ~ AOI*Congruence*Age + (1| Subject), data=response_window_agg, REML=FALSE, control = lmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e6)))
	dev.new()
	...
	aoi_age<-lsmeans(model_time_window_logit,pairwise~Age*AOI|Age,adjust='tukey')
	print(summary(aoi_age)$contrasts)
