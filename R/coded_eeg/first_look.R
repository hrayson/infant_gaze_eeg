...
first_look<-function(data_file, out_file) {
	...
	for(i in 1:length(subj_ids)) {
		...
		for(j in 1:length(trial_ids)) {
			...
			if(trial_data$Target[first_look_target] && trial_data$AntiTarget[first_look_antitarget] && trial_data$Face[first_look_face]) {
				# If looked at target first - set antitarget and face to false
				...
				# If looked at antitarget first - set target and face to false
	...
	first_look_data$Target[first_look_data$Trackloss == TRUE] <- NA
	first_look_data$AntiTarget[first_look_data$Trackloss == TRUE] <- NA
	first_look_data$Face[first_look_data$Trackloss == TRUE] <- NA

	# Save pre-processed data frame to csv (change path accordingly) so don't have to run every time
	write.csv(first_look_data, file = out_file)
}
