gca_adult_target <- function(bin_size, non_aoi_missing, data_file, output_dir) {
	...
	data<-data[data$Condition=='congruent' | data$Condition=='incongruent',]

	data$AdultTarget<-data$Target
	data$AdultAntiTarget<-data$AntiTarget
	data$AdultTarget[data$Condition=='incongruent']<-data$AntiTarget[data$Condition=='incongruent']
	data$AdultAntiTarget[data$Condition=='incongruent']<-data$Target[data$Condition=='incongruent']
	...
	eyetrackingr_data <- make_eyetrackingr_data(data, participant_column = "Subject", trial_column = "Trial", time_column = "TrialTime", trackloss_column = "Trackloss", aoi_columns = c('AdultTarget', 'AdultAntiTarget', 'Face'), treat_non_aoi_looks_as_missing = non_aoi_missing, item_columns = c('Condition','Age'))
	...
	response_time <- make_time_sequence_data(response_window_clean, time_bin_size = bin_size, predictor_columns = c("Condition","Age"), aois = c("AdultTarget","AdultAntiTarget","Face"))
	...
	g <- g + stat_summary(fun.y='mean', geom='line', linetype = 'F1') + stat_summary(fun.data=mean_se, geom='ribbon', alpha= .25, colour=NA)
	g <- g + facet_grid(Condition ~ AOI) + ylab(paste0("Looking to AOI (Prop)"))
	print(g)
	...
	names(summarize_arg) <- 'Prop'
	df_plot <- dplyr::summarize_(df_plot, .dots = summarize_arg)
	g <- ggplot(df_plot, aes_string(x = "Time", y="Prop", group="Condition", color="Condition", fill="Condition")) + xlab('Time in Trial')
	g <- g + stat_summary(fun.y='mean', geom='line', linetype = 'F1') + stat_summary(fun.data=mean_se, geom='ribbon', alpha= .25, colour=NA)
	g <- g + facet_grid(Age ~ AOI) + ylab(paste0("Looking to AOI (Prop)"))
	print(g)
	...
	g <- g + facet_grid(Condition ~ Age) + ylab(paste0("Looking to ", df_plot$AOI[1], " (LogitAdjusted)"))
	print(g)
	...
	print('***** AdultTarget *****')
	response_time <- make_time_sequence_data(response_window_clean, time_bin_size = bin_size, predictor_columns = c("Condition","Age"), aois = c("AdultTarget"))
	# GCA- run model (LogitAdjusted), drop method, plot
	...
	print('***** AdultAntiTarget *****')
	response_time <- make_time_sequence_data(response_window_clean, time_bin_size = bin_size, predictor_columns = c("Condition","Age"), aois = c("AdultAntiTarget"))
	# GCA- run model (LogitAdjusted), drop method, plot
	model_time_sequence <- lmer(LogitAdjusted ~ Condition*Age*(ot1 + ot2 + ot3 + ot4)  + (1+Condition|Subject), data = response_time, REML=FALSE, control = lmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e6)))
	estimate<-broom::tidy(model_time_sequence, effects = "fixed")
	print("Full model (with age factor)")
