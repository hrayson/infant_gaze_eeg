...
output_file = '/home/jbonaiuto/Dropbox/joint_attention/infant_gaze_eeg/preferential_gaze/analysis/eeg/stats.txt'
sink(output_file)
...
# Get second block
pg_data <- pg_data[pg_data$Block==2,]

# Converts data into eyetrackingR format.
eyetrackingr_data <- make_eyetrackingr_data(pg_data, participant_column = "Subject", trial_column = "Trial", time_column = "TrialTime", trackloss_column = "Trackloss", aoi_columns = c('Congruent', 'Incongruent'), treat_non_aoi_looks_as_missing = FALSE, item_columns = c('Age'))
...
response_time <- make_time_sequence_data(response_window_clean, time_bin_size = bin_size, predictor_columns = c("Age"), aois = c("Congruent","Incongruent"))
	...
	for(j in 1:length(ages)) {
		...
		for(k in 1:length(trials)) {
			...
			for(l in 1:length(time_bins)) {
				tb<-time_bins[l]
				diff<-response_time$Prop[response_time$Subject==s & response_time$Age==a & response_time$Trial==t & response_time$TimeBin==tb & response_time$AOI=='Congruent']-response_time$Prop[response_time$Subject==s & response_time$Age==a & response_time$Trial==t & response_time$TimeBin==tb & response_time$AOI=='Incongruent']
				if(length(diff)>0) {
					...
					Time<-c(Time, response_time$Time[response_time$Subject==s & response_time$Age==a & response_time$Trial==t & response_time$TimeBin==tb & response_time$AOI=='Congruent'])
					ot1<-c(ot1, response_time$ot1[response_time$Subject==s & response_time$Age==a & response_time$Trial==t & response_time$TimeBin==tb & response_time$AOI=='Congruent'])
					ot2<-c(ot2, response_time$ot2[response_time$Subject==s & response_time$Age==a & response_time$Trial==t & response_time$TimeBin==tb & response_time$AOI=='Congruent'])
					ot3<-c(ot3, response_time$ot3[response_time$Subject==s & response_time$Age==a & response_time$Trial==t & response_time$TimeBin==tb & response_time$AOI=='Congruent'])
					ot4<-c(ot4, response_time$ot4[response_time$Subject==s & response_time$Age==a & response_time$Trial==t & response_time$TimeBin==tb & response_time$AOI=='Congruent'])
				}
			}
		}
	}
}
df_bias<-data.frame(Subject, Age, Trial, TimeBin, PropDiff, Time, ot1, ot2, ot3, ot4)
df_bias$AOI<-'x'
...
ggsave(file = '/home/jbonaiuto/Dropbox/joint_attention/infant_gaze_eeg/preferential_gaze/analysis/eeg/block2_bias.png')
ggsave(file = '/home/jbonaiuto/Dropbox/joint_attention/infant_gaze_eeg/preferential_gaze/analysis/eeg/block2_bias.eps')
...
#erd_data<-erd_data[erd_data$FreqBand=='mu' & (erd_data$WOI =='0-500ms') & (erd_data$Region=='C'),]
#erd_data<-erd_data[erd_data$FreqBand=='mu' & (erd_data$WOI =='500-1000ms') & (erd_data$Region=='C'),]
#erd_data<-erd_data[erd_data$FreqBand=='mu' & (erd_data$WOI =='1000-1500ms') & (erd_data$Region=='C'),]
#erd_data<-erd_data[erd_data$FreqBand=='mu' & (erd_data$WOI =='1500-2000ms') & (erd_data$Region=='C'),]
erd_data<-erd_data[erd_data$FreqBand=='mu' & (erd_data$WOI =='1000-2000ms') & (erd_data$Region=='C'),]
#erd_data<-erd_data[erd_data$FreqBand=='mu' & (erd_data$WOI =='0-1000ms') & (erd_data$Region=='C'),]
erd_data$Cluster<-''
...
print(g)
# Saves model fit plots as png and eps
ggsave(file = '/home/jbonaiuto/Dropbox/joint_attention/infant_gaze_eeg/preferential_gaze/analysis/eeg/block2_bias-erd.png', width=12, height=7)
ggsave(file = '/home/jbonaiuto/Dropbox/joint_attention/infant_gaze_eeg/preferential_gaze/analysis/eeg/block2_bias-erd.eps', width=12, height=7)
...
g <- g + facet_wrap(~Age) + ylab(paste0("Prop Congruent - Prop Incongruent"))
print(g)
ggsave(file = '/home/jbonaiuto/Dropbox/joint_attention/infant_gaze_eeg/preferential_gaze/analysis/eeg/block2_bias-erd_gca.png', width=12, height=7)
ggsave(file = '/home/jbonaiuto/Dropbox/joint_attention/infant_gaze_eeg/preferential_gaze/analysis/eeg/block2_bias-erd_gca.eps', width=12, height=7)
