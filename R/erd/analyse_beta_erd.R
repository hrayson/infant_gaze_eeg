...
output_file = '/home/jbonaiuto/Dropbox/joint_attention/infant_gaze_eeg/erd/erd_results/obs/beta_stats.txt'
sink(output_file)

#For observation
data <-read.csv(file= '/home/jbonaiuto/Dropbox/joint_attention/infant_gaze_eeg/erd/obs.csv')
data$Subject<-as.factor(data$Subject)
# Select data in beta band, 4 WOIs, and C, O, or F region
#data<-data[data$FreqBand =='beta' & (data$WOI =='0-500ms'|data$WOI =='500-1000ms'|data$WOI == '1000-1500ms'| data$WOI =='1500-2000ms') & (data$Region=='C' | data$Region=='O'| data$Region=='F'),]
data<-data[data$FreqBand =='beta' & (data$WOI =='0-500ms'|data$WOI =='500-1000ms'|data$WOI == '1000-1500ms'| data$WOI =='1500-2000ms') & (data$Region=='C' | data$Region=='O'| data$Region=='F'| data$Region=='P'),]
...
g<-ggplot(data_summary, aes(x=Cluster, y=ERD, fill=Condition))+facet_grid(WOIf~Age)+geom_bar(position=dodge, stat="identity")+geom_errorbar(aes(ymin=ERD-se,ymax=ERD+se),width=0.4,position=dodge)+geom_text(position=dodge, aes(y=ERD-se,label=star), colour="red", vjust=1.25, size=5)+theme_bw(base_size=12)+ylim(-30,10)
print(g)

# Saves model fit plots as png and eps
ggsave(file = '/home/jbonaiuto/Dropbox/joint_attention/infant_gaze_eeg/erd/erd_results/obs/beta_erd.png', width=12, height=7)
ggsave(file = '/home/jbonaiuto/Dropbox/joint_attention/infant_gaze_eeg/erd/erd_results/obs/beta_erd.eps', width=12, height=7)
...
# When including P
age1<-lsmeans(basic_model,pairwise~Age)
print(summary(age1)$contrasts)

# When including P
woi1<-lsmeans(basic_model,pairwise~Condition*WOI|WOI)
...
output_file = '/home/jbonaiuto/Dropbox/joint_attention/infant_gaze_eeg/erd/erd_results/exe/beta_stats.txt'
sink(output_file)

#For execution
data <-read.csv(file= '/home/jbonaiuto/Dropbox/joint_attention/infant_gaze_eeg/erd/exe.csv')
data$Subject<-as.factor(data$Subject)
# Select data in beta band, 2 WOIs, and C, O, or F region
#data<-data[data$FreqBand =='beta' & (data$WOI == '-500-0ms'| data$WOI =='0-500ms') & (data$Region=='C' | data$Region=='O' | data$Region=='F'),]
data<-data[data$FreqBand =='beta' & (data$WOI == '-500-0ms'| data$WOI =='0-500ms') & (data$Region=='C' | data$Region=='O' | data$Region=='F' | data$Region=='P'),]
# Rename hemisphere/region combinations to clusters
data$Cluster<-''
data$Cluster[data$Region == 'C' & data$Hemisphere == 'left']<-'C3'
data$Cluster[data$Region == 'C' & data$Hemisphere == 'right']<-'C4'
...
# Saves model fit plots as png and eps
ggsave(file = '/home/jbonaiuto/Dropbox/joint_attention/infant_gaze_eeg/erd/erd_results/exe/beta_erd.png')
ggsave(file = '/home/jbonaiuto/Dropbox/joint_attention/infant_gaze_eeg/erd/erd_results/exe/beta_erd.eps')
