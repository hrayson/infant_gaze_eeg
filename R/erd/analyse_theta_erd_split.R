...
output_file = '/home/jbonaiuto/Dropbox/joint_attention/infant_gaze_eeg/erd/erd_results/obs/theta_stats_split.txt'
sink(output_file)

#For observation
data <-read.csv(file= '/home/jbonaiuto/Dropbox/joint_attention/infant_gaze_eeg/erd/obs_saccade_cue_all_theta_split.csv')
...
data_summary<-summarySE(data, measurevar="ERD", groupvars=c("Condition","WOI","Age","Half"))
	...
	for(k in 1:length(wois)) {
		...
		for(l in 1:length(ages)) {
			...
			for(m in 1:length(halves)) {
				half<-halves[m]
				print(half)
				t_data <- data[data$Condition==condition & data$WOI==woi & data$Age==age & data$Half==half,]
				res<-t.test(t_data$ERD)
				print(res)
				data_summary$p[data_summary$Condition==condition & data_summary$WOIf==woi & data_summary$Age==age & data$Half==half]<-res$p.value
			}
		}
	}
}
...
g<-ggplot(data_summary, aes(x=Half, color=Condition, group=Condition, y=ERD))+facet_grid(WOIf~Age)+geom_bar(position=dodge, stat="identity")+geom_errorbar(aes(ymin=ERD-se,ymax=ERD+se),width=0.4,position=dodge)+geom_text(position=dodge, aes(y=ERD-se,label=star), colour="red", vjust=1.25, size=5)+theme_bw(base_size=12)+ylim(-45,20)
print(g)

# Saves model fit plots as png and eps
ggsave(file = '/home/jbonaiuto/Dropbox/joint_attention/infant_gaze_eeg/erd/erd_results/obs/theta_erd_split.png', width=12, height=7)
ggsave(file = '/home/jbonaiuto/Dropbox/joint_attention/infant_gaze_eeg/erd/erd_results/obs/theta_erd_split.eps', width=12, height=7)
...
g<-ggplot(data_summary,aes(x=WOIf,y=ERD,color=Condition,linetype=Age,group=interaction(Condition,Age)))+geom_errorbar(aes(ymin=ERD-se, ymax=ERD+se), width=.1, color='black', position=pd)+geom_line(position=pd)+geom_point(position=pd,size=3,shape=21,fill='white')+facet_grid(~Half)+xlab('Time window')+ylab('ERD')+theme_bw()+theme(legend.justification=c(0,1),legend.position=c(0,1))
print(g)

# Saves model fit plots as png and eps
ggsave(file = '/home/jbonaiuto/Dropbox/joint_attention/infant_gaze_eeg/erd/erd_results/obs/theta_erd_fmedial_all_split.png', width=12, height=7)
ggsave(file = '/home/jbonaiuto/Dropbox/joint_attention/infant_gaze_eeg/erd/erd_results/obs/theta_erd_fmedial_all_split.eps', width=12, height=7)
